...
- **Before finishing / concluding a feature** — two things must both be true:
  ...
  2. A **complete, clean run of the single command `scripts/test-all.sh`** succeeds, covering **all 5 tiers**:
     ...
     - Tier 3: Docker E2E (mandatory — runs against `pgweb/postgres:latest` + full `examples/todo/` flows; the script auto-detects when extension sources, Dockerfile, or init scripts are newer than the image and triggers `scripts/build-image.sh`)
