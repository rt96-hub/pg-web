...
- **Stack:** `pgweb/postgres:latest` (the shipped artifact), bench-specific `bench/docker-compose.yml` (shape copied from `examples/todo/`), published :8080 for the load generator on the host. In-image `pg-web` CLI used for push (`--with-migrate` on first run). `env = "production"` (no dev error pages, no livereload LISTEN).
